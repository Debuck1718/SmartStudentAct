    ...
    const updatedUser = await User.findByIdAndUpdate(
      userId,
      {
        $set: {
          ...
          nextBillingDate: eventData?.nextBillingDate || null,
        },
      },
      { new: true }
    );
      ...
      if (event.event === "charge.success") {
        ...
        if (userId) {
          const eventData = {
            ...
            receiptLink: event.data.receipt_number
              ? `https://paystack.com/receipt/${event.data.receipt_number}`
              : null,
...
export const handleFlutterwaveWebhook = (req, res) => {
  try {
    ...
    if (
      ...
      if (userId) {
        const eventData = {
          ...
          receiptLink: event.data.flw_ref
            ? `https://flutterwave.com/receipt/${event.data.flw_ref}`
            : null,
