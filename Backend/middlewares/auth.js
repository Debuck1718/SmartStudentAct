...
// Create a regular expression from the public routes for a more flexible match
const publicRoutesRegex = new RegExp(`^(${PUBLIC_ROUTES.join('|')})/?$`);
...
    // Skip authentication for whitelisted routes using the regex
    if (publicRoutesRegex.test(req.path)) {
        return next();
    }
...
            if (err) {
                // Log the token verification error for debugging
