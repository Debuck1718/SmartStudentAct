...
module.exports = (eventBus, agenda) => {
  ...
  const signupOtpSchema = Joi.object({
    ...
    teacherGrade: Joi.alternatives().try(Joi.number().integer().min(5).max(12), Joi.string().valid("100","200","300","400","500","600")).allow(null),
  ...
  const contactSchema = Joi.object({ name: Joi.string().min(2).max(100).required(), email: Joi.string().email().required(), message: Joi.string().min(5).max(2000).required() });
  ...
  // --- Signup: send OTP ---
  publicRouter.post("/users/signup-otp", rateLimit({ windowMs: 5*60*1000, max: 3 }), validate(signupOtpSchema), async (req, res) => {
  ...
  publicRouter.post("/users/verify-otp", validate(verifyOtpSchema), async (req, res) => {
    ...
    if (Date.now() - signupData.timestamp > 10*60*1000) { delete req.session.signup; return res.status(400).json({ error: "OTP expired" }); }
