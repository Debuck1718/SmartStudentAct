  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (!receiverId || !requestType) {
        return res.status(400).json({
          message: "receiverId and requestType are required.",
        });
      }

      const existing = await SpecialLinkRequest.findOne({
        requester_id: userId,
      ...
      const newRequest = new SpecialLinkRequest({
        requester_id: userId,
        receiver_id: receiverId,
      ...
      return res.status(201).json({
        message: "Special link request sent successfully.",
        request: newRequest,
      });
    }
    ...
    if (req.method === "GET") {
      ...
      if (type === "sent") {
        ...
        return res.status(400).json({
          message: "Invalid type. Use ?type=sent or ?type=received",
        });
      }
    ...
    res.status(500).json({
      message: "Server error while handling special link request.",
      error: err.message,
    });
  }
}
